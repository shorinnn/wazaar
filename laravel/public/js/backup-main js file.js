...
function skinVideoControls(){
    /*var video = document.getElementsByTagName('video')[0];
     ...
     var playButton = $('.play-button, .centered-play-button');
     var pauseButton = $('.pause-button');
     var durationContainer = $('.duration');
     var currentTimeContainer = $('.current');
     $('.video-controls, .progress').width(videoWidth);

     // Create play and pause states
     ...
     $(currentTimeContainer).text((video.currentTime/60).toFixed(2) + ' / ');
     }, 1000);*/

    //Log out current playing time and total duration
    /*$(video).on('timeupdate', function () {
     var minuteSide = Math.floor(video.currentTime/60);
     var secSide = Math.round(video.currentTime%60);
     if(secSide.length == 1){
     secSide = '0' + secSide;
     }
     else if(secSide.length > 1){
     secSide = secSide;
     }
     $(currentTimeContainer).text(minuteSide + ':' + secSide + ' / ');
     console.log(Math.round(video.currentTime%60) + ' / ');
     $('.progress-bar').attr("value", this.currentTime / this.duration);
     })
     $(durationContainer).text((video.duration).toFixed(2));
     ...
     if(video.fullscreenElement || video.webkitFullscreenElement || video.mozFullScreenElement || video.msFullscreenElement){
     video.controls = true;
     }
     else{
