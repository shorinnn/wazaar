...
class InstructorAgency extends User {
    ...
    public function creditReverse( $transaction ){
        if( $transaction->transaction_type!='instructor_agency_credit' || $transaction->user_id != $this->id || $transaction->status!='complete' ){
            return false;
        }
        $old = $transaction;
        return DB::transaction(function() use ($old){
            // create the transaction
              ...
              $transaction->transaction_type = 'instructor_agency_credit_reverse';
              $transaction->details = trans('transactions.instructor_agency_credit_reverse_transaction').' '.$old->reference;

//              $transaction->reference = $order;
              $transaction->status = 'complete';
              if( $transaction->save() ){
                  // increase balance
                  $this->agency_balance -= $old->amount;
