...
//            $student->answers = $student->receivedMessages()->where('type','ask_teacher')->where('course_id', $course->id)->orderBy('id','desc')->get();
            ...
            if( $course==null ) return Redirect::to('/');
            if( $course->free == 'no' && !$student->purchased( $course ) && 
                    !$student->purchasedLessonFromCourse( $course ) &&
                    $course->instructor_id != Auth::user()->id &&
                    $course->assigned_instructor_id != Auth::user()->id  ){
                return Redirect::to('/');
            }
            if( $course->free == 'yes' && !$student->purchased( $course ) && 
                    !$student->purchasedLessonFromCourse( $course ) &&
                    $course->instructor_id != Auth::user()->id &&
                    $course->assigned_instructor_id != Auth::user()->id && !Auth::user()->hasRole('Affiliate')  ){
                return Redirect::to('/');
            }
...
//            if($video!=null) $video = $video->first();
            ...
            $discussions = $student->discussions()->where('course_id', $course->id)->paginate(5);
