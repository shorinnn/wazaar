...
class CourseCategorySeeder extends Seeder {
    ...
    public function run()
    {
        ...
        CourseCategory::create( ['name' => 'Health', 'slug' => 'health',  'graphics_url' => 'https://wazaar.s3.amazonaws.com/assets/images/misc-images/misc-icons-13.png',
           ...
           /** extra categories **/        
            for($i = 0; $i<10; ++$i){
                CourseCategory::create( ['name' => 'Extra Category '.$i, 'slug' => 'extra-category-'.$i,  'graphics_url' => 'https://wazaar.s3.amazonaws.com/assets/images/misc-images/misc-icons-13.png',
                                 'description' => 'Fat it up bro!', 'courses_count' => 0, 'color_scheme' => 7]);
            }
           /** end extra categories **/        
...
class CourseSubcategorySeeder extends Seeder {
    ...
    public function run()
    {
        ...
        $i = 0;
        foreach(CourseCategory::all() as $cat){
            CourseSubcategory::create( ['name' => 'DummySubCat '.$i, 'slug' => 'dummy-subcat-'.$i, 
                                 'description' => 'Dummy Subcat '.$i, 'courses_count' => 0, 'course_category_id' => $cat->id ]);
            ++$i;
        }
    }
...
', 
        ...
        for($i = 0; $i<400; ++$i){
            $cat = CourseCategory::find( rand(7,15 ) );
        Course::create( ['name' => 'Dummy course '.$i, 'slug' => 'dummy-course-'.$i, 'instructor_id' => 11, 'course_category_id' => $cat->id, 
                        'course_subcategory_id' => $cat->courseSubcategories->first()->id, 'price' => rand(500,1000), 'course_difficulty_id' => '2', 
                        'description' => 'Dummy Course '.$i, 
                        'student_count' => 0, 'privacy_status' => 'public', 'affiliate_percentage' => 0, 'requirements' => '[]',
                        'who_is_this_for' => '["Beginners that don’t know anything about C++ ","Existing who want to pick up javascript."]',
                        'what_will_you_achieve' => '["Something", "Something Else!"]']);
        }
...
class ModulesSeeder extends Seeder {
    ...
    public function run()
    {
        ...
        $exclude = [1, 5, 14];
        foreach(Course::whereNotIn('id',$exclude)->get() as $course){
            for($i = 1; $i < rand(4, 5); ++$i){
                Module::create( ['course_id' => $course->id, 'name' => 'Module Number '.$i, 'order' => $i] );
            }
        }
...
class LessonsSeeder extends Seeder {
    ...
    public function run()
    {
        ...
        $exclude = [1, 5, 14];
        foreach(Course::whereNotIn('id',$exclude)->with('modules')->get() as $course){
            foreach($course->modules as $module){
                for($i = 1; $i < rand(4, 6); ++$i){
                     Lesson::create( ['module_id' => $module->id, 'name' => 'Lesson #'.$i, 'order' => $i, 'description' => 'Arms',
                         "published" => 'yes', 'price' => rand(0, 100)] );
                }
            }
        }
    }
...
class BlocksSeeder extends Seeder {
    ...
    public function run()
    {
        ...
        foreach(Lesson::whereNotIn('id', [10])->get() as $lesson){
            Block::create( ['lesson_id' => $lesson->id, 'name' => 'Test Block 1 Lesson '.$lesson->id,'type' => 'text', 
            'content' => 'Test Block 1 Lesson '.$lesson->id] );
            Block::create( ['lesson_id' => $lesson->id, 'name' => 'Test Block 2 Lesson '.$lesson->id,'type' => 'text', 
            'content' => 'Test Block 2 Lesson '.$lesson->id] );
            Block::create( ['lesson_id' => $lesson->id, 'name' => 'First File','type' => 'file', 'mime' => 'image/jpeg', 
                'key' => 'course_uploads/file-5526bab93ea21.jpg',
                'content' => 'https://wazaardev.s3.amazonaws.com/course_uploads/file-5526bab93ea21.jpg'] );
            Block::create( ['lesson_id' => $lesson->id, 'name' => 'First Video Ever','type' => 'video', 'content' => '1'] );
        }
...
class TestimonialsSeeder extends Seeder {
    ...
    public function run()
    {
        ...
        foreach(Course::all() as $course){
            for($i = 0; $i < 5; ++$i){
                Testimonial::create( ['course_id' => $course->id, 'student_id' => rand(20,90), 'rating'=>'positive', 'thumbs_up' => 100, 
                'thumbs_down' => 12,
                'content' => 'TESTIMONIAL '.$i.'下半身太りでむくみが気になる体質なのでバストアップだけでなくその解決方法もしれて良かったです！'] );
            }
        }
...
class PMSeeder extends Seeder {
    ...
    public function run()
    {
        ...
        for($i = 0; $i< 500; ++$i){
            PrivateMessage::create( ['sender_id' => rand(2, 200), 'recipient_id' => rand(2, 200), 'thread_id' => 0, 
                'type' => 'student_conversation', 'course_id' => '0', 'lesson_id' => 10, 'content' => 'Extra PM '.$i] );
        }
...
class TransactionsSeeder extends Seeder {
    ...
    public function run()
    {
        ...
        $students = Student::where('id', '>', 20)->limit(100)->get();
            $paymentData = [];
            $paymentData['successData']['processor_fee'] = '0';
            $paymentData['successData']['tax'] = '10';
        ...
        foreach($students as $student){
            $paymentData['successData']['REF'] = 'c-'.rand(999,9999);//.$student->id
            $paymentData['successData']['ORDERID'] = 'c-'.rand(999,9999);
            $courses = Course::limit( rand(1, 4) )->get();
...
//            $paymentData['successData']['REF'] = 'l-'.rand(999,9999);
//            $paymentData['successData']['ORDERID'] = 'l-'.rand(999,9999);
//            $lessons = Lesson::limit( rand(1, 2) )->get();
//            foreach($lessons as $lesson){
//                $student->purchase($lesson, null, $paymentData);
//            }
        }
    }
}
